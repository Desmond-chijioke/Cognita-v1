...
import { LuUser, LuBuilding2, LuMail, LuPhone, LuTag, LuSave, LuCamera, LuLock } from 'react-icons/lu';
...
export default function PGSchoolSettings() {
  ...
  const [profile, setProfile] = useState({ name: user?.name || '', title: 'PG Coordinator', phone: '' });
  ...
  return (
    <Box p="xl">
      <Box mb="xl">
        ...
        <Text size="sm" c="dimmed" mt={4}>Manage your profile and account preferences.</Text>
      </Box>

      <Stack gap="lg">
        {/* Profile card */}
        <Paper withBorder radius="md" bg="white" style={{ overflow: 'hidden' }}>
          <Box style={{ position: 'relative', height: 100, background: 'linear-gradient(120deg, #6741d9 0%, #a855f7 100%)' }}>
            <Box
            ...
            >
              <Avatar src={avatarUrl ?? undefined} color="grape" radius="xl" size={84}
                style={{ border: '3px solid white', boxShadow: '0 6px 20px rgba(103,65,217,0.3)', fontSize: 26, fontWeight: 700 }}>
              ...
              <Box style={{ position: 'absolute', bottom: 2, right: 2, width: 26, height: 26, borderRadius: '50%', background: '#6741d9', border: '2px solid white', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <Box px="xl" pb="xl">
            <Group justify="space-between" align="center" mt={52} mb="lg" wrap="nowrap">
              <Box>
                ...
                <Text size="sm" c="dimmed" mt={2}>{profile.title} · Postgraduate School</Text>
              </Box>
            ...
            <SimpleGrid cols={{ base: 1, sm: 2, lg: 3 }} spacing="md" mb="md">
              ...
              <TextInput label="Title / Position" placeholder="e.g. PG Coordinator" leftSection={<LuTag size={14} color="#868e96" />}
        ...
        <Paper withBorder p="xl" radius="md" bg="white">
          <SectionHeader icon={LuBuilding2} title="Institution Information" subtitle="Your institution assignment (read-only)" />
          <SimpleGrid cols={{ base: 1, sm: 2 }} spacing="md">
        ...
        <Paper withBorder p="lg" radius="md" bg="white">
          <Group justify="space-between" align="center">
            ...
            <Button color="grape" size="md" leftSection={<LuSave size={15} />} loading={saving} onClick={handleSave} px="xl">
              Save Settings
            </Button>
          </Group>
        </Paper>
