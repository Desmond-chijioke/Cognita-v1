...
export default function CollegeSettings() {
  ...
  const collegeName     = user?.departmentName  ?? '';
  ...
  const [profile, setProfile] = useState({ name: user?.name || '', title: 'Provost', phone: '' });
  ...
  return (
    <Box p="xl">
      <Box mb="xl">
        <Title order={2} style={{ fontFamily: 'Playfair Display, serif' }}>College Settings</Title>
        <Text size="sm" c="dimmed" mt={4}>Manage your profile and college configuration.</Text>
      </Box>
      ...
      <Stack gap="lg">
          ...
          <Box px="xl" pb="xl">
            <Group justify="space-between" align="center" mt={52} mb="lg" wrap="nowrap">
              <Box>
                ...
                <Text size="sm" c="dimmed" mt={2}>{profile.title} · {collegeName}</Text>
              </Box>
            ...
            <SimpleGrid cols={{ base: 1, sm: 2, lg: 3 }} spacing="md" mb="md">
              ...
              <TextInput label="Title / Position" placeholder="e.g. Provost" leftSection={<LuTag size={14} color="#868e96" />}
        ...
        <Paper withBorder p="xl" radius="md" bg="white">
          <SectionHeader icon={LuBuilding2} title="College Information" subtitle="Your college and institution details" />
          <SimpleGrid cols={{ base: 1, sm: 2 }} spacing="md">
            <TextInput label="College Name" value={collegeName} readOnly
