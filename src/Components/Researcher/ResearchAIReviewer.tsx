...
import {
  ...
  LuArrowRight, LuRefreshCw, LuX, LuShield, LuFileText,
} from 'react-icons/lu';
import type { IssueSeverity } from '../Student/studentData';
...
function buildPrompt(sections: { id: string; title: string; content: string }[]): string {
  return `You are an academic writing reviewer giving feedback on a research document.
...
2. List specific issues found, each with:
   ...
   - "suggestion": a concrete, actionable rewrite or fix (or null if the issue doesn't need one)
  ...
  return (
    <Box>
      <Group justify="space-between" align="flex-start" mb="sm">
        <Box>
          <Text size="sm" fw={600}>Choose sections to review</Text>
          <Text size="xs" c="dimmed">The AI gives feedback only on the sections you select below.</Text>
        </Box>
        <Group gap={6}>
...
export default function ResearchAIReviewer() {
  ...
  const handleRun = async () => {
    ...
    if (chosen.length === 0) {
      notifications.show({ title: 'Nothing selected', message: 'Choose at least one section to review.', color: 'orange' });
      return;
    }
    ...
    try {
      ...
      notifications.show({ title: 'Review complete', message: 'AI review updated for your selected sections.', color: 'brand' });
    } catch (err) {
    ...
    <Box p="xl">
      <Group justify="space-between" align="flex-start" mb="xl">
        <Box>
          ...
          <Text size="sm" c="dimmed" mt={4}>AI-driven quality review of your saved research drafts.</Text>
        </Box>
        ...
        <Paper withBorder p="lg" radius="md" bg="white" mb="xl">
          ...
          <Group justify="flex-end" mt="md">
            <Button color="brand" leftSection={<LuRefreshCw size={14} />} loading={running}
              disabled={drafts.length === 0 || selected.size === 0} onClick={handleRun}>
      ...
      {loading ? null : !report ? (
        <Paper withBorder p="xl" radius="md" bg="white" ta="center">
          ...
          <Text size="sm" c="dimmed">Select sections above and run a review to get AI-driven feedback and a category breakdown.</Text>
        </Paper>
      ) : (
        <>
          {/* ── Score summary ── */}
          <SimpleGrid cols={{ base: 1, md: 2 }} mb="xl">
            ...
            <Paper withBorder p="xl" radius="md" bg="white">
              ...
              <Text size="xs" c="dimmed">Run a new review to update scores based on your latest saved drafts.</Text>
            </Paper>
          </SimpleGrid>
